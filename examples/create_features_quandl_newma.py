...
from settings.default import (
    QUANDL_TICKERS,
    CPD_NEWMA_QUANDL_OUTPUT_FOLDER,
    FEATURES_NEWMA_QUANDL_FILE_PATH,
)
from mom_trans.data_prep import (
    deep_momentum_strategy_features,
    include_changepoint_features_newma,
)
...
def main(
    ...
    if lookback_window_length:
        features_w_cpd = include_changepoint_features_newma(
        ...
        if extra_lbw:
            # for extra in extra_lbw:
                raise ValueError(
                    "Extra lookback window lengths are not supported in this version."
                )
                # extra_data = pd.read_csv(
                #     output_file_path.replace(
                #         f"quandl_cpd_bocd_{lookback_window_length}lbw.csv",
                #         f"quandl_cpd_bocd_{extra}lbw.csv",
                #     ),
                #     index_col=0,
                #     parse_dates=True,
                # ).reset_index()[
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        return (
            QUANDL_TICKERS,
            CPD_NEWMA_QUANDL_OUTPUT_FOLDER(args.lookback_window_length),
            args.lookback_window_length,
            FEATURES_NEWMA_QUANDL_FILE_PATH(args.lookback_window_length),
            args.extra_lbw,
        )
